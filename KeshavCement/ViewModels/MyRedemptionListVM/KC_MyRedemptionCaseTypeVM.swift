//
//  KC_MyRedemptionCaseTypeVM.swift
//  KeshavCement
//
...
class KC_MyRedemptionCaseTypeVM {
    
    weak var VC: KC_RedemptionTypeVC?
    ...
    func myRedemptionListApi(parameter: JSON){
        ...
        self.requestAPIs.cashTransferRedemptionTypeList(parameters: parameter) { (result, error) in
            if error == nil{
                if result != nil{
                    DispatchQueue.main.async {
                        self.VC?.stopLoading()
