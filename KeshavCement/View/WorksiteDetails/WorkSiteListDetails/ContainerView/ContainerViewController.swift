    ...
    override open func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == segueIdentifier{
            ...
            UIView.transition(with: self.view, duration: animationDurationWithOptions.0, options: animationDurationWithOptions.1, animations: {
                ...
                if let vc = segue.destination as? KC_UserDetailsVC {
                    vc.currentLatitude = self.currentLatitude
                    vc.currentLongitude = self.currentLongitude
                    vc.strdata1 = self.strdata1
                    vc.siteName = self.siteName
                    
                   }
                if let vc = segue.destination as? KC_WorkDetailsVC {
                    vc.strdata1 = self.strdata1
                    vc.currentLatitude = self.currentLatitude
                    vc.currentLongitude = self.currentLongitude
                    vc.siteName = self.siteName
                    vc.ownerName = self.ownerName
                    vc.ownerMobile = self.ownerMobile
                    vc.ownerResidentialDetails = self.ownerResidentialDetails
                    vc.engineerName = self.engineerName
                    vc.engineerNumber = self.engineerNumber
                   }
